...
void ColorduttObject::LED_Delay(unsigned char i)
{
  unsigned int y;
  y = i * 10;
  while(y--);
...
// wbval[1]=green
// wbval[2]=blue
void ColorduttObject::SetWhiteBal(unsigned char wbval[3])
{
  LED_LAT_CLR;
  LED_SLB_CLR;
  for(unsigned char k=0;k<ColorduttScreenHeight;k++)
...
********************************************************/
void ColorduttObject::ColorFill(unsigned char R,unsigned char G,unsigned char B)
{
  PixelRGB *p = GetPixel(0,0);
  for (unsigned char y=0;y<ColorduttScreenWidth;y++) {
    for(unsigned char x=0;x<ColorduttScreenHeight;x++) {
      p->r = R;
      p->g = G;
      p->b = B;
      p++;
...
ISR(TIMER4_OVF_vect)          //Timer4  Service
{
  ...
  close_all_lines;
  Colordutt.run();
  Colordutt.open_line(Colordutt.line);
  if (++Colordutt.line > 7) Colordutt.line = 0;
}
#else
...
void ColorduttObject::run()
{
  LED_SLB_SET;
  LED_LAT_CLR;
  PixelRGB *pixel = GetDrawPixel(0,line);
  for(unsigned char x=0;x<ColorduttScreenWidth;x++)
  {
    unsigned char temp = pixel->b;
    unsigned char p;
    for(p=0;p<ColorduttBitsPerColor;p++) {
      if(temp & 0x80)
	      LED_SDA_SET;
      else
	      LED_SDA_CLR;
